...
export function beforeSubmit(
  ...
  if (context.type === context.UserEventType.CREATE) {
    ...
    log.debug(
      "beforeSubmit",
      `Submitting new transaction for entity: ${customerId}`,
    ); // When creating a transaction from an entity, log the entity internal id
    ...
    log.debug(
      "beforeSubmit",
      `Customer ${customerId} values: ${JSON.stringify(results)}`,
    );
  }
}
